...
public class ChambreDAO implements IChambreDAO {
    ...
    public ChambreDAO() throws SQLException {
        ...
        try {
            ...
            String createTableSQL = "CREATE TABLE IF NOT EXISTS chambre (" +
        ...
        String requetSQL = "SELECT c.* FROM chambre c " +
                " INNER JOIN chambre_hotel ch ON c.id_chambre = ch.id_chambre " +
                " WHERE ch.id_hotel = ? AND c.est_disponible = true AND c.numero_personnes >= ?;";
        try {
    ...
    @Override
    public ChambreList getChambresDisponibles(long idHotel, LocalDate dateArrivee, LocalDate dateDepart, int nbPersonnes) {
        return null;
    }
}
